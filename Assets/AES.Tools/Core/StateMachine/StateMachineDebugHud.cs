...
namespace AES.Tools.StateMachine
{
    public sealed class StateMachineDebugHud : MonoBehaviour
    {
        ...
        private Core.StateMachine _machine;
        ...
        private void TryPickTargetFromClick()
        {
            ...
            {
                if (UnityEngine.Input.touchCount > 0) pointerPos = UnityEngine.Input.GetTouch(0).position;
                else pointerPos = UnityEngine.Input.mousePosition;
            }
#else
        if (UnityEngine.Input.touchCount > 0)
